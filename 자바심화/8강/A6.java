...
public class A6 {
	...
	public static void main(String[] args) throws IOException {
		...
		File k43_f1 = new File("C:\\Users\\kopo\\Desktop\\df\\day_data\\StockDailyPrice.csv");// 저장할 파일 클래스
		...
		int k43_cnt = 0;
		int k43_wcnt = 0;
		while ((readtxt = k43_br.readLine()) != null) {// 한줄씩 읽을 때마다 비어있지 않으면
			StringBuffer k43_s = new StringBuffer();// 스트링 버퍼 선언
			String[] k43_field = readtxt.split("%_%");// %_%를 기준으로 나눈다

			if (k43_field.length > 2 && k43_field[2].replace("^", "").trim().substring(0, 1).equals("A")) {// 헤더가 2보다 크고
																											// ^를 빈칸으로
																											// 바꾸고 단축코드가
																											// A라면
				...
				for (int k43_j = 1; k43_j < k43_field.length; k43_j++) {
					k43_s.append("," + k43_field[k43_j].replace("^", "").trim());//^를 빈칸으로 교체 하고 공백을 제거
				}
				k43_bw.write(k43_s.toString());//문자열로 바꾸고
				k43_bw.newLine();//줄바꿈
				k43_wcnt++;// THTSKS010H00.dat 에서 원한 줄 +1
			}
			k43_cnt++;// THTSKS010H00.dat 모든줄 +1
		}
		k43_br.close();
		k43_bw.close();
